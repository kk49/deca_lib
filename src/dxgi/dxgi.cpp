...
struct RGBA
{
    ...
    u8 & operator[](int index)
    {
        switch(index)
        {
            case 0: return this->r_;
            case 1: return this->g_;
            case 2: return this->b_;
            case 3: return this->a_;
            default: return this->r_;
        }
    }
...
extern "C" {
    ...
    u8* alloc_bin(u64 sz)
    {
        if(sz > buffer_in_sz)
        {
            if(buffer_in_ptr != nullptr)
                free(buffer_in_ptr);
            buffer_in_ptr = (u8*)malloc(sz);
            buffer_in_sz = sz;
        }
        return buffer_in_ptr;
    }

    EMSCRIPTEN_KEEPALIVE
    u8* alloc_bout(u64 sz)
    {
        if(sz > buffer_out_sz)
        {
            if(buffer_out_ptr != nullptr)
                free(buffer_out_ptr);
            buffer_out_ptr = (u8*)malloc(sz);
            buffer_out_sz = sz;
        }
        return buffer_out_ptr;
    }
